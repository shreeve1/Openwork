...
- When sandboxing blocks the deployment network calls, rerun with `network-enabled shell permissions`.
...
If deployment fails due to network issues (timeouts, DNS errors, connection resets), rerun the deploy with escalated permissions (use `network-enabled shell permissions`). The deploy requires escalated network access when sandbox networking blocks outbound requests.
